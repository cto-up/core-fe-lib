...
const emits = defineEmits<{
  (
    e: "update:sort",
    payload: { column: string; value: "asc" | "desc" | undefined }
  ): void;
...
const toggleSort = (val: boolean | undefined) => {
  ...
  if (val == undefined) {
    emits("update:sort", {
      column: Object.keys(props.column)[0],
      value: undefined,
    });
  ...
  >
    <DropdownMenu>
      <DropdownMenuTrigger as-child>
        ...
        >
          <span>{{ title }}</span>
          <Icon v-if="currentSort" name="ArrowDown" class="ml-2 h-4 w-4" />
          <Icon
            v-else-if="currentSort == undefined"
            name="ChevronsUpDown"
            class="ml-2 h-4 w-4"
          />
          <Icon
            v-else-if="currentSort == false"
            name="ArrowUp"
            class="ml-2 h-4 w-4"
          />
        </Button>
