import { type VariantProps, cva } from "class-variance-authority";

export { default as Button } from "./Button.vue";

export const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        // Primary buttons carry the brand gradient (violet→fuchsia, derived
        // from --primary so tenant branding re-tints it). Opt-in: consumers
        // that don't define --brand-grad-* fall back to a solid --primary,
        // so other core-fe-lib apps are unaffected.
        default:
          "text-primary-foreground shadow-sm transition-all bg-[linear-gradient(100deg,hsl(var(--brand-grad-from,var(--primary))),hsl(var(--brand-grad-to,var(--primary))))] hover:opacity-90",
        destructive:
          "bg-destructive text-destructive-foreground hover:bg-destructive/90",
        success: "bg-success text-success-foreground hover:bg-success/90",
        info: "bg-info text-info-foreground hover:bg-info/90",
        warning: "bg-warning text-warning-foreground hover:bg-warning/90",
        outline:
          "border border-input bg-background hover:bg-accent hover:text-accent-foreground",
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-secondary/80",
        ghost: "hover:bg-accent hover:text-accent-foreground",
        link: "text-primary underline-offset-4 hover:underline",
      },
      size: {
        // xs was in use at three call sites before it existed here, so those
        // buttons silently got no size class at all and relied on whatever the
        // caller passed in `class`.
        xs: "h-7 rounded-md px-2 text-xs",
        default: "h-10 px-4 py-2",
        sm: "h-9 rounded-md px-3",
        lg: "h-11 rounded-md px-8",
        // 44px on a phone, 40 from md up. An icon-only button carries no label
        // to widen its target, so at 40px square it sits under every published
        // touch minimum — and these are the controls that sit next to the text
        // they act on (an overflow menu beside a title, a close beside a
        // heading), which is exactly where a near-miss hits the wrong thing.
        // The navbar was already hand-writing this pair at three call sites.
        icon: "h-11 w-11 md:h-10 md:w-10",
      },
    },
    defaultVariants: {
      variant: "default",
      size: "default",
    },
  }
);

export type ButtonVariants = VariantProps<typeof buttonVariants>;
