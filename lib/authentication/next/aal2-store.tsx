...
export function Aal2Provider({ children }: { children: React.ReactNode }) {
  ...
  const manager = useMemo(() => new Aal2Manager(stateAdapter), [stateAdapter]);
  ...
  const updateState = useCallback((updates: Partial<Aal2VerificationState>) => {
    setState((prev) => ({ ...prev, ...updates }));
  }, []);
  ...
  return <Aal2Context.Provider value={value}>{children}</Aal2Context.Provider>;
}
