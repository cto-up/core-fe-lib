...
export function useMfa() {
  ...
  const [recoveryCodesInteracted, setRecoveryCodesInteracted] = useState(false);
  ...
  const setupTOTP = useCallback(async () => {
    try {
      ...
      if (totpNode?.attributes?.src) {
        ...
        setTotpSecretKey(secret || "Scan QR code with your authenticator app");
      } else {
  ...
  const generateRecoveryCodesForSetup = useCallback(async () => {
    try {
      ...
      const codes = extractRecoveryCodes(response as Record<string, unknown>);
  ...
  const confirmRecoveryCodesAndFinish = useCallback(async () => {
    try {
      ...
      console.log("✅ Confirming recovery codes with flow:", settingsFlowId);
